...
const AcquireKeyConfigurationRegister = ({ fetchDataAction, tableData }) => {
  useEffect(() => {
    fetchDataAction();
  }, [fetchDataAction]);
  let selectionData = {};
  ...
  const handleSubmit = async (formValues) => {
    ...
    //window.alert("Successfull Configured Acquirer Keys: " + `${JSON.stringify(response, null, 2)}`);
  
    if (response.status === 200) {
      window.alert("Successfull Configured Acquirer Keys");
      setIsRedirect(true);
    }
    else
  ...
  if (isRedirect) {
    return <Redirect to="/softpos/merchant/keyConfig/acquirerkey/register" />;
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(AcquireKeyConfigurationRegister);